...
Following the adoption of UN Resolution 65/309 in 2011, the UN has released since 2012, with the exception of 2014, a year report on global happiness.
...
These happiness scores are a result of a yearly survey conducted under the auspicious of the UN.
...
Each predictor will now be defined.
...
####Happiness
The target, happiness, is the national average of ranking a ranking of how well a person believes their life is going, from the worst possible, to the best possible.

####Economy
Economy is a measure of national GDP per capita in PPP per capita in constant 2011 dollars in August of 2016. 

####Family
Family is the average of a indicator variable measuring whether or not the respondent has someone, a friend or family member, who would help them in a time of trouble.

####Health
Health is a measure of healthy life expectancy at birth, not simply the life expectancy at birth.

####Freedom
Freedom is the average of an indicator variable of whether or not the respondent is satisfied with their freedom to choose what to do in life.

####Generosity
Generosity is a measure of the frequency of charitable donation per month scaled with GDP per capita.

####Trust
Trust is a measure of corruption perception, the average between indicators of existence of widespread public or private corruption. 

###Analysis
In order to better understand the relationship between the different predictors, first the data will be explored. This exploration will look for any problems with the data itself, either missing or questionable values. Correlations between the predictors will be calculated to determine if multicolinearity will be an issue with this data. 

Following that the relationship between the predictors will be measured using various unstructured learning techniques. Principle Component Analysis (PCA), will be done to better understand the relationship between the different predictors, and to serve as predictors if multicolinearity proves intractable. A Bi-Plot will the be created to identify outlier nations that require special consideration and interpretation. Hierarchical clustering will then be done to help to identify patterns among the nations in the survey, these patterns will also be identified through K- Means Clustering analysis. 

Subsequent to unstructed learning, structured learning, namely predictive modeling, with be done to predict happiness using the 6 other predictors identified for this analysis. 3 models will be fit to the data: LASSO, Random Forest (RF), and Support Vector Machine (SVM). These three techniques will undergo hyperparameter optimization, model, validation, and finally model comparison to the select the best fit for the target. 
...
```
The scree plot indicates that 3 principle components(PC) needs to be interpreted. The 1^st^ principle component:
```{r}
...
####Myanmar
Myanmar is ranked low, 114 out of 155. Its outlier nature is due to its much higher then expected generosity score given its happiness-- it has the highest score. The most likely explanation for this discrepancy is the highly Therevadan Buddhist nature of the majority Bamar ethnic group who, by the definition of the predictor, must be more likely to help others. 

####Rwanda
Rwanda is also ranked very low, 151 out of 155. It's outlier nature is due to its very trust score given its happiness, the second highest. This is most likely due to the after effects of the Rwandan genocide, and the extreme changes it brought to the Rwandan government, eliminating a large amount of the corruption endemic to the region.

####Somalia
Somalia is ranked near the middle in terms of happiness, 93. However, Somalia's economy score is second to last, which would usually indicate an extremely low happiness rating. The reason for this difference is most likely the continued factional nature of the Somali state. It might also indicate a need to address the adequacy of economic indicators in Somalia, and lead to questions about their accuracy. 
...
```
While a RF is said to be self-validating, given the nature of the algorithm, `r its` 70/30 train/test splits were done to estimate the MSE and $\sigma$ of the estimates. 500 trees were used. Once hyperparameter optimization is coded, this number will be determined by optimization of the fit of the data. The results are as follows:
...
It is clear from the tables that Health, then Economy, then Family and finally Freedom, in that order play an important role in predicting happiness in a country. Generosity and Trust appear to lack much predictive power. Interpreting these results, the most important factor in predicting happiness is how long an individual is, and how healthy they are during their life. After that how wealthy, then how much support, then how free an individual is to make choices round out the most important predictors. 

If we were to engage in sophomoric psychology, we would find these correspond rather well to Maslow's Hierarchy of needs. \footnote{Maslow, A (1954). Motivation and personality. New York, NY: Harper.} The most important need is that for corporeal safety, i.e., safety in one's body. Following that economic safety, the ability to earn enough money to support one's self. Then moving on to the ability to find aid in times of trouble, and at last being free to choose how one uses their life. 
